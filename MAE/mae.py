...
#
# Example:
#   python mae.py --device cuda --epochs_pre 100 --epochs_ft 50 --batch_size 128 --amp
# Quick test:
#   python mae.py --device cpu --epochs_pre 2 --epochs_ft 2 --mask_ratios 0.5 --log_interval 100
...
class MAEViT(nn.Module):
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        B, C, H, W = imgs.shape
        target = imgs
        # reconstruct pixel space per patch (normalize per patch like MAE)
...
def train_mae_epoch(model, dataloader, optimizer, scaler, lr_schedule, wd, epoch, device, mask_ratio, log_interval=100):
    model.train()
    total_loss = 0.0
    n = 0
    running = 0.0
    for it, (imgs, _) in enumerate(dataloader):
        ...
        if (it + 1) % log_interval == 0 or (it + 1) == len(dataloader):
            avg_loss = running / min(log_interval, (it + 1))
            print(f"[Pretrain][Epoch {epoch+1}] Step {it+1}/{len(dataloader)} | LR {optimizer.param_groups[0]['lr']:.2e} | Loss {avg_loss:.4f}", flush=True)
            running = 0.0
...
def train_classifier(model, train_loader, test_loader, cfg: TrainConfig):
    ...
    for epoch in range(cfg.epochs_ft):
        ...
        for it, (imgs, targets) in enumerate(train_loader):
            ...
            running += loss.item()
            if (it + 1) % cfg.log_interval == 0 or (it + 1) == len(train_loader):
                avg_loss = running / min(cfg.log_interval, (it + 1))
                print(f"[Finetune][Epoch {epoch+1}] Step {it+1}/{len(train_loader)} | LR {lr:.2e} | CE {avg_loss:.4f}", flush=True)
                running = 0.0

        acc = evaluate(model, test_loader, device)
        if acc > best_acc:
            best_acc = acc
        print(f"[Finetune] Epoch {epoch+1}/{cfg.epochs_ft} - test acc: {acc*100:.2f}% (best {best_acc*100:.2f}%)", flush=True)
    return best_acc
...
def run_experiment(mask_ratios: List[float], cfg: TrainConfig, workdir: str = "./outputs"):
    ...
    for mr in mask_ratios:
        ...
        for ep in range(cfg.epochs_pre):
            loss = train_mae_epoch(mae, train_loader, optimizer, scaler, lr_sched, cfg.wd_pre, ep, device, mask_ratio=mr, log_interval=cfg.log_interval)
            print(f"[Pretrain] Epoch {ep+1}/{cfg.epochs_pre} | Avg Loss: {loss:.4f}", flush=True)
        ...
        mae.to(device)
        print(f"Loaded MAE encoder from checkpoint: {enc_path}")
...
def parse_args():
    ...
    # 원래는 위에가 맞는데 그지같은 윈도우 업데이트때문에 다 날아감
    # 진짜 방에서 걷다가 발가락 찧어라
    # p.add_argument("--mask_ratios", type=float, nargs="+", default=[0.5, 0.75], help="MAE mask ratios to try")
    p.add_argument("--log_interval", type=int, default=100, help="print progress every N batches")
    args = p.parse_args()
