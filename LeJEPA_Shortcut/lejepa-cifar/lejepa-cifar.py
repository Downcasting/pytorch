...
from src.model_data import ViTEncoder, CIFARDataset
...
def main(cfg: DictConfig):
    ...
    wandb.init(project="LeJEPA-CIFAR10-shortcut", config=dict(cfg), id=wandb_id, resume="allow" if wandb_id else None)
    torch.manual_seed(0)
    ...
    if pretrain_mode:
        ...
        train_ds = CIFARDataset("train", V=cfg.V, mode='clean')
    else:
        print("Pretrain with colored dataset")
        train_ds = CIFARDataset("train", V=cfg.V)

    test_ds = CIFARDataset("test", V=1)
    test_ds_clean = CIFARDataset("test", V=1, mode='clean')
    ...
    # [수정/추가됨] 논문(On the Stepwise Nature of SSL)의 Small Initialization 적용
    small_init_alpha = cfg.get("small_init_alpha", 1.0) # 기본값 1.0, 실험 시 0.1 ~ 0.0001 등으로 설정
    # 체크포인트에서 가중치를 불러오는 상황이 아닐 때만 초기화 스케일링 적용
    if small_init_alpha < 1.0 and not os.path.exists(checkpoint_path):
        print(f"Applying Small Initialization with alpha={small_init_alpha}")
        with torch.no_grad():
            # LayerNorm 등의 파라미터는 건드리지 않고, Linear와 Conv 계층의 가중치만 축소하는 것이 안정적입니다.
            for m in net.modules():
                if isinstance(m, (nn.Linear, nn.Conv2d)):
                    m.weight.data.mul_(small_init_alpha)
                    if m.bias is not None:
                        m.bias.data.mul_(small_init_alpha)


    # ViT-Tiny has 192 features, not 512
    probe = nn.Sequential(nn.LayerNorm(192), nn.Linear(192, 10)).to("cuda")
